...
Likewise `BaseClaude`, `BaseCodex`, and `BaseOpenCode` ship as built-in **agents**: a step
may name any of them in its `agent:` with no `registry.yml` entry. The compiler injects the
referenced one (a fixed harness on the reserved `default` sandbox, no skills) so a project
can drive a Claude Code, Codex, or OpenCode runtime out of the box. A user who declares an
agent of the same name overrides the built-in — their definition wins.
...
# Built-in agents ship one per runtime; a step names one by `agent:` with no registry entry.
...
# BaseOpenCode's model carries the `provider/` prefix the opencode runtime requires.
...
BUILTIN_AGENTS: dict[str, dict[str, str]] = {
    ...
    "BaseOpenCode": {"runtime": "opencode", "model": "anthropic/claude-sonnet-4-6"},
}
