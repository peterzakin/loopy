...
| **SandboxProvider** | Provisions the compute + egress an agent runs in, from the sandbox spec (image build + `network:` allowlist). The trust boundary. | `local` (bare subprocess, for dev), `docker` (hermetic local container from the spec's `image:`), or `daytona` (isolated cloud container). Selected per-sandbox via `provider:` in `registry.yml` — the runtime routes each step to the backend its sandbox names; there is no launch-time flag. |
