"""env_file addendum: a sandbox references an env file by path; the compiler never reads it.

The referenced file supplies the sandbox's secrets (parsed and injected at run time); the
compiler records only the path(s) and serializes them into the manifest.
"""
