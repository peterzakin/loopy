...
def test_request_tolerates_non_dict_json_error_body():
    """A body like `["boom"]` has no `.get` — `_request` must not raise AttributeError."""
    from loopy_cli.render import RenderAPIError
...
class _HijackClient(_ScriptedClient):
    """find_service turns up a same-named service that belongs to a different repo."""

    def find_service(self, name):
        self.calls.append(("find", name))
        return {"id": "srv-other", "name": name, "repo": "https://github.com/other/project"}


def test_deploy_render_refuses_hijack_on_repo_mismatch(tmp_path, monkeypatch):
    """A same-name service found via find_service (not the recorded id) must be repo-checked
    before any env/secrets/deploy call — otherwise --service-name collisions hijack a
    stranger's Render service."""
    from typer.testing import CliRunner
    ...
    def factory(key):
        scripted["client"] = _HijackClient(key)
        return scripted["client"]
    ...
    client = scripted["client"]
    kinds = [c[0] for c in client.calls]
    assert kinds == ["find"]  # no create/env/secrets/deploy
    assert "other/project" in result.output
    assert "acme/demo" in result.output


def test_deploy_render_headless_warnings_exit_names_the_flag(tmp_path, monkeypatch):
    """Headless + an unaccepted warning must exit 1 and tell the operator about --yes."""
    from typer.testing import CliRunner
    ...
    monkeypatch.setattr(
        render_mod,
        "project_checks",
        lambda root, manifest: [Check("dirty", "working tree is committed", False, warn=True, fix="commit it")],
    )
...
def test_deploy_render_update_path_notes_ignored_region_flag(tmp_path, monkeypatch):
    """--region on an existing service is a create-only flag — note it, don't PATCH."""
    from typer.testing import CliRunner
    ...
    result = CliRunner().invoke(
        app, ["deploy", "render", str(project), "--root", str(project), "--region", "frankfurt"]
    )
    assert result.exit_code == 0, result.output
