...
_WEBHOOK_CONVERSION = {"id": 7, "pem": "PEM", "slug": "loopy-abcd", "webhook_secret": "whsec_9"}


def test_run_github_auth_registers_webhook_from_registry_public_url(monkeypatch, tmp_path):
    # registry.yml's `public_url` (collected at init, editable any time) flows into the App
    # manifest as the webhook URL, and the secret GitHub mints round-trips into loopy.env.
    (tmp_path / "registry.yml").write_text("public_url: https://loopy.example.com/\n")
    captured: dict = {}
    _stub_auth_flow(monkeypatch, captured, _WEBHOOK_CONVERSION)
...
def test_run_github_auth_env_var_overrides_registry(monkeypatch, tmp_path):
    # LOOPY_PUBLIC_URL is a deploy-time constant — set in the process env it wins over
    # whatever registry.yml carries.
    (tmp_path / "registry.yml").write_text("public_url: https://stale.example.com\n")
    captured: dict = {}
    _stub_auth_flow(monkeypatch, captured, _WEBHOOK_CONVERSION)
    monkeypatch.setenv("LOOPY_PUBLIC_URL", "https://live.example.com")

    auth.run_github_auth(root=tmp_path, no_browser=True)

    assert captured["webhook_url"] == "https://live.example.com/hooks/github"


def test_run_github_auth_explicit_public_url_wins(monkeypatch, tmp_path):
    # An explicit public_url (the init wizard's just-collected answer, or --public-url)
    # beats every configured source — and is normalized on the way in.
    (tmp_path / "registry.yml").write_text("public_url: https://stale.example.com\n")
    captured: dict = {}
    _stub_auth_flow(monkeypatch, captured, _WEBHOOK_CONVERSION)
...
def test_run_github_auth_without_public_url_stays_webhook_less(monkeypatch, tmp_path):
    ...
    captured: dict = {}
    _stub_auth_flow(
        monkeypatch, captured, {"id": 7, "pem": "PEM", "slug": "loopy-abcd", "webhook_secret": None}
    )
...
def test_run_github_auth_no_webhook_flag_opts_out(monkeypatch, tmp_path):
    # --no-webhook keeps the App a pure credential source even with a URL configured.
    (tmp_path / "registry.yml").write_text("public_url: https://loopy.example.com\n")
    captured: dict = {}
    _stub_auth_flow(monkeypatch, captured, {"id": 7, "pem": "PEM", "slug": "x"})

    auth.run_github_auth(root=tmp_path, no_browser=True, no_webhook=True)
