...
| **B6** | **Budget enforcement.** Enforce `budget.wall_clock` (minutes), per-step `budget.spend.usd`, and cumulative cascade caps that terminate runaway loop-backs — `--max-tokens` (universal) and `--max-spend` (USD, gated on cost-reporting harnesses); honor `window`/`latency` (days). | Runs span days; must be bounded. |
