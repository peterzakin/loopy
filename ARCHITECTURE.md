...
| **B6** | **Budget enforcement.** Enforce `budget.wall_clock` (minutes), per-step `budget.spend.usd`, and a cumulative cascade **token** cap (`--max-tokens`) that terminates runaway loop-backs; honor `window`/`latency` (days). | Runs span days; must be bounded. |
