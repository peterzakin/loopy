...
_REGISTRY_YML = """\
# __PROJECT_NAME__ — a starter Loopy project. A CodeTask event drives a claude-code agent
# that edits a checkout and opens a PR. `loopy compile .` validates any edit.

defaults:
  agent:
    sandbox: BaseSandbox

# Compute + egress for every agent. `daytona` runs each in an isolated cloud sandbox
# (set DAYTONA_API_KEY in loopy.env); swap `provider:` to `docker` for a local run.
sandboxes:
  BaseSandbox:
    provider: daytona
    image: { debian_slim: "3.12", apt: [git], workdir: /home/loopy, user: loopy }
    network: [github.com, api.anthropic.com]   # add api.openai.com to run a step on Codex
    env_file: secrets/base.env                 # gitignored; injected at run time
    __REPOS_LINE__

# One agent per harness; a step picks one with `agent:`. `model` is what it runs on,
# `harness` the runner. Codex/OpenCode also need their key (secrets/base.env) + API host.
agents:
...
# The bus contract: every event a step may name in `on:` or `emits:`.
events:
  CodeTask:       # fire by hand: loopy trigger --event CodeTask --fields '{...}'
    task: str     # what to change, in prose
    branch: str   # the branch to push the edit on
  PROpened:       # emitted once the PR is open; a downstream workflow could `on:` it
    pr_url: url
    summary: str
"""
...
def task_queue(req) -> CodeTask:
    ...
    A stub: a real sensor would read from Linear, a GitHub issue label, a spreadsheet, etc.,
    and return a CodeTask per request (None emits nothing). It gives `CodeTask` a declared
    producer; you can also fire one by hand with `loopy trigger --event CodeTask`.
    \"\"\"
    return None
"""

_DEV_ENV = """\
# The BaseSandbox sandbox's environment, injected at run time. Gitignored — never commit.
# Lines are KEY=VALUE; values are literal (no ${VAR} interpolation, no inline comments).

# Model auth (required for daytona/docker — the image carries no creds).
ANTHROPIC_API_KEY=sk-ant-...
# OPENAI_API_KEY=sk-...     # the Codex agent (or OpenCode on an OpenAI model) uses this

# Git auth — only if NOT using a GitHub App. `loopy auth github` injects a scoped token;
# otherwise put a PAT (contents:write + pull_requests:write) here.
# GITHUB_TOKEN=ghp_...
...
_LOOPY_ENV = """\
# Control-plane credentials — what the loopy *engine* needs. NOT injected into sandboxes
# (that's secrets/base.env); these stay at the control plane. Gitignored — never commit.

# GitHub App (recommended git auth). Leave commented: `loopy auth github` fills these in,
# and an uncommented GITHUB_APP_ID makes it think an App is already configured.
# GITHUB_APP_ID=
# GITHUB_APP_PRIVATE_KEY=

# Daytona (required when a sandbox uses provider: daytona).
# DAYTONA_API_KEY=

# Redis event bus. Set to run on the networked bus; unset = single-process in-memory bus.
# REDIS_URL=redis://localhost:6379

# Public webhook base URL — a deployed host or dev tunnel (ngrok, cloudflared) where external
# services deliver webhooks. `loopy run` prints the full delivery URLs at startup.
# LOOPY_PUBLIC_URL=

# Admin dashboard bearer token — gates /admin on any non-loopback bind. `loopy init` mints one
# here; when you deploy, copy this exact value into the platform's secrets.
# LOOPY_ADMIN_TOKEN=
"""
...
# __PROJECT_NAME__ — a minimal Loopy registry, initialized without GitHub access, so there is
# no starter workflow yet. Loopy agents work on repos (clone, edit, open a PR); before building:
#   1. wire git auth: `loopy auth github`
#   2. add the repo(s) to sandboxes.BaseSandbox.repos below
#   3. add a workflow under workflows/<name>/ — `loopy docs` has the authoring reference
# `loopy compile .` validates after every edit.
...
sandboxes:
  BaseSandbox:
    ...
    network: [github.com, api.anthropic.com]   # add api.openai.com to run a step on Codex
    env_file: secrets/base.env                 # gitignored; injected at run time
    # repos: [owner/repo]                       # set once `loopy auth github` has run
...
# The bus contract: register every event a workflow will `on:` or `emits:`. For example:
# events:
#   CodeTask:
#     task: str
#     branch: str
